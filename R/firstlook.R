...
look<-function(x,forceType=TRUE) {
	ret <- list()
	...
	class(ret)<-"AISFirstLook"
	return (ret)
}


print.AISFirstLook<-function(x,...) {
  nvar<-dim(x$data)[2]
	if (is.null(nvar)) nvar<-1
   if (nvar==1) {
...
plot.AISFirstLook<-function(x,single=FALSE, ask=dev.interactive(),...) {
  ...
  nvar <- dim(x$data)[2]
	...
	if (nvar==1) {
	  ...
	  heatmap(x$cor,margins=c(10,10),revC=T,scale="none",breaks=seq(-1,1,length=13),...)
	  legend("topleft",c("1","0","-1"), fill=c(   "#FFFFD5FF", "#FF9F00FF", "#FF0000FF") )
		if (!single) {
		   if (is.ts(x$data)) {
          plot(x$data,main="")
		   }
